...
> **2026-09-07 产品定位修正：** 以 [部署助手职责](deployment-contract.md) 为准。兼容原目录、原启停命令；仅保存上一版应用压缩包用于失败回退，不管理迁移或数据备份。下文涉及强制 releases/current、空目录接管及多版本管理的旧要求已被取代；旧验收不代表原目录发布已实现。
...
2. 支持前端、后端或二者联合构建与部署。
3. 兼容现有应用目录和启停命令，不要求业务改造为版本目录和软链接发布。
4. 完整记录部署步骤、命令输出、版本信息和操作结果。
5. 保存上一版应用压缩包，用于本次发布失败时恢复。
