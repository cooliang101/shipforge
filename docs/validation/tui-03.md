...
TUI-04 继续管理页面一致性；TUI-05 审查全部运行/退出路径，包含既有部署规划线程的关闭跟踪；后续 [TUI-06](tui-06.md) 测量高吞吐下与 UI 排空解耦的有界淘汰、整进程内存与合成输入延迟。早期未定位 SSH 超时仍由 M4/QA-01 复核，本阶段不宣称已解决。现行 MVP 仅支持 Windows，macOS 和最低 Rust 版本矩阵不再列为发布门禁。
