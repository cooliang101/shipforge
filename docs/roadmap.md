...
1. **轻量安全复核**：检查防误删、凭据/日志、Host Key、上传校验及失败结果处理；复用已有测试，只为发现的具体缺口补回归。已知的误删、泄密或错误报告成功问题仍须修复，不扩展企业级安全工程。
2. **易用性与交付**：按实际使用反馈修正 TUI 输入、默认值与错误提示；交付固定路径 `target/release/shipforge.exe`，补齐简短的运行、更新和失败处理说明，不强制制作安装器或自动发布系统。
3. **简短验收**：核对实际项目部署记录、配置再次打开可复用、代表性失败处理和终端退出；代码变更通过现有本机门禁，每个完成的小阶段单独提交。
