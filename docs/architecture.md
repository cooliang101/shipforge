...
The accepted MVP scope includes configurable remote Service Commands, with systemd as a built-in Service Preset over the same execution mechanism. This is not implemented yet: the current code has a dedicated systemd path. `SVC-01` is the next development package, before `QA-02` and final release gates; existing systemd acceptance does not validate custom commands. Remote dependency installation remains outside scope.
...
Project configuration uses names and short forms; generated immutable Project/Environment IDs plus per-Component generations and resolved default roots live in `_shipforge`. YAML mapping order has no execution meaning, and optional Component `after` dependencies are topologically sorted with deterministic tie-breaking. The following example is the currently supported schema; `SVC-01` must implement and document the canonical service-command shape before replacing it:
...
Filesystem activation is atomic per Component, not across Components or an Environment. Observation accepts only a non-dangling `current -> releases/<version>` link whose version directory is not itself a link. Activation verifies the prepared archive and directory types, dereferences filesystem identities, checks expected `current` both before and after creating a Deployment-specific temporary link, then renames that link over `current`. After this switch, service completion and compensation use independent bounded tokens so user cancellation cannot strand half-finished recovery. Compensation re-observes `current` and refuses to overwrite drift. Implemented checks currently include Destination-side HTTP/HTTPS and systemd stability; `SVC-01` adds optional read-only command checks for custom services, including services without HTTP endpoints. If a Component fails, the Driver restores that Component's previous `current`; if none existed, it removes the new link and stops the newly started service. Components already activated by the same Deployment are compensated in reverse order.
...
### Service Commands and Presets — accepted, next implementation

`SVC-01` replaces the dedicated systemd service path inside `linux-ssh`, not the Driver SPI or the release/transfer model. The base abstraction is a validated, frozen service-command plan; a systemd preset supplies commands and observation rules to that same plan. Preset-specific interpretation of active/NRestarts remains necessary, but transport, execution, logging, cancellation and compensation are shared. PM2 is a custom-command use case, not another Driver or a new automatic-discovery subsystem.
...
Configuration field names and conversion fixtures are finalized with implementation in `SVC-01`; no planned keys are accepted by the current binary. Windows tests, disposable real Linux custom/PM2 lifecycle tests, and systemd regressions must pass before resuming release gates. See [the next work package](roadmap.md#svc-01远端服务命令基础与-systemd-预设).
