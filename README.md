...
M1's safe deployment loop has passed acceptance, but this is not a production-ready release. Setup, deployment planning, live logs, cancellation, and the core rollback service are implemented. Real two-Destination deployment and [WSL systemd lifecycle tests](docs/validation/systemd-acceptance.md) pass. M2 now provides [local detailed history](docs/validation/his-01.md) and [read-only remote Release inventory with append-only audit](docs/validation/his-02.md). Recovery, retention, history management screens, final TUI hardening, and the complete platform matrix remain on the [roadmap](docs/roadmap.md).
