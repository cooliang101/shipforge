...
M1's safe deployment loop has passed acceptance, but this is not a production-ready release. Setup, deployment planning, live logs, cancellation, and the core rollback service are implemented. Real two-Destination deployment and [WSL systemd lifecycle tests](docs/validation/systemd-acceptance.md) pass. M2's [HIS-01 local history model](docs/validation/his-01.md) now persists frozen plans, Release metadata, timed steps and explicit observations. Remote inventory/recovery/retention, history management screens, final TUI hardening, and the complete platform matrix remain on the [roadmap](docs/roadmap.md).
