...
The same isolated password fixture covers explicit saved-password sudo prompts, rejection, cached/no-prompt execution, hostile echo suppression, cancellation and timeout. It never changes server sudo policy or contacts user destinations. See [sudo password validation](../docs/validation/sudo-password.md).
