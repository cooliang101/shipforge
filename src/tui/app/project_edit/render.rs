...
impl ProjectEditScreen {
    ...
    pub(in crate::tui) fn render(&self, frame: &mut Frame<'_>, area: Rect) {
        ...
        let paragraph = Paragraph::new(body)
            .block(
                Block::default()
                    .borders(Borders::ALL)
                    .title(format!(" {} ", crate::tui::i18n::tr(title))),
            )
            .scroll((scroll, 0));
    ...
    fn content(&self) -> (&'static str, String, Option<usize>) {
        match &self.page {
            ...
            ProjectEditPage::Text(edit) => ("Edit one value", crate::tui::i18n::format!("{}\n\n{}\n\nNo IDs or secrets should be entered. Changes remain in memory until YAML preview is confirmed.", "{}\n\n{}\n\n无需输入 ID 或密码。确认 YAML 预览前，修改仅保存在草稿中。", edit.field.label(), safe_text(&edit.value)), None),
            ProjectEditPage::Discard => ("Discard unsaved draft?", crate::tui::i18n::tr("No file has been saved. Press c to discard all in-memory edits; Esc keeps editing.").into(), None),
            ProjectEditPage::Delete(kind) => ("Confirm draft removal", delete_text(kind), None),
            ProjectEditPage::Preview(preview) => ("Confirm exact shipforge.yaml", crate::tui::i18n::format!("{}\n--- EXACT YAML TO SAVE ---\n{}", "{}\n--- 即将保存的完整 YAML ---\n{}", preview_summary(preview), preview.yaml()), None),
            ...
            ProjectEditPage::Discovery { report, cursor } => {
                ...
                for (index, candidate) in report.components.iter().enumerate() { let _ = writeln!(text, "{} {} · {:?} · {}", mark(index == *cursor), candidate.name, candidate.confidence, safe_text(&crate::tui::presentation::path_label(&candidate.source))); }
    ...
    fn home(&self) -> String {
        let Some(draft) = &self.draft else {
            return crate::tui::i18n::tr("Configuration is unavailable. Press r to retry, or Esc to return. No file was created.").into();
        };
        crate::tui::i18n::format!(
            ...
            "项目：{}\n目录：{}\n{}\n\n修改先保存在草稿中，此处不会构建、连接 SSH、发布或删除远端内容。\n内部 ID 自动维护，无需输入 ID 或密码。",
            safe_text(&draft.setup.project),
            safe_text(&crate::tui::presentation::path_label(&self.root)),
            if self.dirty {
                crate::tui::i18n::tr("UNSAVED DRAFT")
            } else {
                crate::tui::i18n::tr("Saved configuration loaded; no changes yet")
            }
        )
    }

    fn components(&self, cursor: usize) -> (&'static str, String, Option<usize>) {
        let mut text = crate::tui::i18n::tr(
            ...
            for (index, (name, component)) in draft.setup.components.iter().enumerate() {
                let _ = crate::tui::i18n::writeln!(
                    ...
                    component.working_directory.as_ref().map_or_else(
                        || format!("{name}/ (default)"),
                        |path| safe_text(&crate::tui::presentation::path_label(path))
                    ),
                    safe_text(&crate::tui::presentation::path_label(
    ...
    fn form_content(&self) -> (&'static str, String, Option<usize>) {
        match &self.page {
            ProjectEditPage::Component { form, cursor } => component_form(form, *cursor),
    ...
    fn environment_form(
        ...
        let mut rows = vec![crate::tui::i18n::format!(
            "Environment name: {}{}",
            "环境名称：{}{}",
        ...
        rows.push(crate::tui::i18n::tr("Apply Environment to draft (not YAML save)").into());
        (
    ...
    fn target_form(
        ...
        let rows = [
            crate::tui::i18n::format!("Connection: {destination}", "连接：{destination}"),
            crate::tui::i18n::format!("Remote root: {root}", "远端目录：{root}"),
            crate::tui::i18n::format!(
                "Service: {}",
                "服务：{}",
                form.target
            ...
            crate::tui::i18n::tr("Apply target to Environment form").into(),
        ];
        (
    ...
    (
        ...
        fields(
            &[
                ...
                crate::tui::i18n::tr("Apply Component to draft (not YAML save)").into(),
            ],
            cursor,
        ),
        Some(cursor),
...
fn preview_summary(preview: &ProjectEditPreview) -> String {
    ...
    let mut text = format!(
        ...
        safe_text(&crate::tui::presentation::path_label(
            &preview.root().join("shipforge.yaml")
        )),
